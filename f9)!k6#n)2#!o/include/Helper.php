...
{
    include('env.php')
    $app_id = $app_id??$_SESSION[$app_key.'_user_active_app_id'];
    $tables = [];
    try {
...
public function getTablesWithSizes($app_id = null)
{
    ...
    try {
        ...
        $stmt = $conn->prepare('SELECT table_name AS `Table`, round(((data_length + index_length) / 1024 / 1024), 2) `Size` FROM information_schema.TABLES WHERE table_schema = "'.$app_db.'" AND table_name LIKE "app'.$app_id.'\_%"');
        $stmt->execute();
        $raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
public function getFields($table, $skips, $app_id = null)
{
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '".'app'.$app_id.'_'.$table."'");
        $stmt->execute();
        $raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
public function getAppFields($skips)
{
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT DISTINCT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME LIKE 'app".$this->app_id."\_%'");
        $stmt->execute();
        $raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
public function getAppFieldsOfDataTypes($data_types)
{
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT DISTINCT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME LIKE 'app".$this->app_id."\_%' AND DATA_TYPE IN ('".implode(', ', $data_types)."')");
        $stmt->execute();
        $raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
public function getUserStorageFootPrint($app_id = null)
{
    ...
    try {
        ...
        foreach ($app_ids as $id) {
            $stmt = $conn->prepare('SELECT SUM(round(((data_length + index_length) / 1024 / 1024), 2)) `Size` FROM information_schema.TABLES WHERE table_schema = "'.$this->con.'" AND table_name LIKE "app'.$id.'\_%"');
            $stmt->execute();
            $raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
