...
<script>
  ...
  function updateMsg(){
  	$.post('{{ route('c.chat.message.update') }}',{"_token":"<?php echo $rand; ?>","id":$("#chat_id").val(),"_method":"put","message":$("#message").val(),"style":$("#style").val(),"status":$("#status").val(), 'cmd':'status_only' },function(data, status){
  		if(status == 'success'){
