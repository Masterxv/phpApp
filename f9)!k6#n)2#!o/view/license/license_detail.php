...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-md-6">
      <a class="btn btn-default" style="float:right" href="/license/license_list">Back</a> 
    </div>
  </div>
  <div class="row">
    <div class="col-md-12">
      ...
      <table class="table">
        ...
        <tbody>
          <?php foreach($licenseDetails as $k => $licenseDetail): ?>
            <tr>
              <td><?php echo  ($k + 1) + 10 * ($pageno-1); ?></td>
              <td><?php echo $license_id; ?></td>
              <td><?php echo $license_key; ?></td>
              <td><?php echo $licenseDetail['hardware_code']; ?></td>
              <td><?php echo $licenseDetail['computer_name']; ?></td>
              <td><?php echo $licenseDetail['computer_user']; ?></td>
              <?php if($licenseDetail['hardware_code'] == "Empty"): ?>
              <td><a href="JavaScript:void(0);" onclick="sr(<?php echo $k; ?>)" data-toggle="modal" data-target="#manualActivation">Activate</a></td>
              <?php else: ?>
              <td><a href="JavaScript:void(0);" onclick="sr(<?php echo $k; ?>)" >De-Activate</a></td>
              <?php endif; ?>
            </tr>
          <?php endforeach; ?>
        </tbody>
...
<script>
  ...
  function activate(){
    ...
    $.post("/license/activate", ainp, function(data){
  ...
  function de_activate(){
    ...
    $.post("/license/deactivate", dinp, function(data){
